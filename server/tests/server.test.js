...
describe('GET /todos', () => {
  it('should get all todos for valid user', (done) => {
    request(app)
      .get('/todos')
      .set('x-auth', users[0].tokens[0].token)
      .expect(200)
  ...
  describe('GET /todos/:id', () => {
    ...
    describe('PATCH /todos/:id', () => {
      it('should update the todo', (done) => {
        ...
        it('should not update the todo of invalid users', (done) => {
          var hexId = todos[0]._id.toHexString();
          var changedText = 'This shits tested';
          //auth as 2nd user
          var completedTime = todos[0].completedAt = 345;
...
describe('POST /users', () => {
  ...
  it('should not create user if email in use', (done) => {
    //use taken email
    //should fail with valid password expect 400
    request(app)
    .post('/users')
    .send({
      email: users[0].email,
...
describe('DELETE /users/me/token', () => {
  it('should remove auth token on logout', (done) => {
    ...
    .delete('/users/me/token')
    //set x-auth equal to token in tokens array
    .set('x-auth', users[0].tokens[0].token)
    //expect 200
    .expect(200)
    //async call to find users, verify tokens array has length of 0
    .end((err, res) => {
      if (err) {
        return done(err);
      }
